...
- **Citation Hardening (May 2026 v2):** Reinforces token grounding with four layers of validation: (1) **Regex hardening** — widened `_TOKEN_FULL`/`_TOKEN_INLINE` regexes tolerate punctuation/whitespace/missing-underscore variants (`REF_5.`, `Ref 5`, `(REF_5)`, `"REF_5"`), enabling recovery of malformed tokens from non-Anthropic models. (2) **Multi-token sources** — supports `[REF_3, REF_4]` and `REF_3 and REF_5` syntax via `finditer()`. First match becomes primary source; additional matches stored in `additional_sources[]` field (future UI expansion). (3) **Per-claim backfill** — replaces section-level fallback with `_best_article_for_claim()` using token-overlap scoring between claim text and article titles/abstracts. Distinct claims in same section can now land on different articles (fixes "refs disappear" symptom). (4) **LLM-ref validation** — `_is_grounded_ref()` cross-checks LLM-supplied references against `ref_map` (PMID/NCT/DOI/normalized-title match); ungrounded hallucinations dropped before frontend. URL verification in `url_builder.py` ensures LLM-provided URLs exist in fetched-article set (else nulled + rebuilt from PMID/NCT). (5) **Quarantine** — `_quarantine_sourceless_items()` drops content items and references lacking source/url/pmid after backfill, guaranteeing zero orphaned claims. Explicit enum-style schema instruction lists valid `[REF_1]...[REF_{max_n}]` tokens in data block preamble.
