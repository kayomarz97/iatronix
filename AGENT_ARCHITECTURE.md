...
- **Known residual (the next lever):** symptom-anchored entities ("profuse vomiting", "anxiety") still match a symptom-named wrong chapter — an ENTITY/differential problem (retrieve on candidate diagnoses), not a chapter-match one. Belongs to the F3 differential path / the real analyzer.
