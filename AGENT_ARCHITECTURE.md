...
## Server
- Public IPv4: REDACTED_HOST_IP
- IPv6: REDACTED_HOST_IPV6
...
- med.kayomarz.com   → DEVELOPMENT (dev git branch, Docker dev compose)
...
| backend/app/services/ranking.py | Evidence quality ranker — multi-factor scoring (study type, relevance, recency, fulltext, citations) with penalties for animal/off-population studies. `rank_article_list(use_synonyms, apply_floor, min_keep)`: **relevance floor** drops entity-absent articles (keep ≥ min_keep); **synonym** matching (paracetamol⇄acetaminophen). **`apply_topicality_gate()`** (F2, `TOPICALITY_GATE_ENABLED`): keeps ONLY subject-mentioning articles (prefix-stem match), never re-admits off-topic — the *aboutness* net. Flag-gated, default OFF |
...
| backend/app/services/differential_dx.py | **NEW** (F3, `DIFFERENTIAL_DX_ENABLED`) — `is_differential_query()` / `differential_terms()`: for "what could this finding be?" queries, adds etiology/differential-sense PubMed terms anchored on the FINDING so retrieval targets candidate diagnoses, not single-disease treatment literature |
...
### Differential-diagnosis mis-anchor fix — 4 levers (2026-07-21, dev-first, prod OFF)
Root cause of "answer about a random pancreatic-cancer drug for `abdominal mass with mets to spleen differential diagnosis`": the fetch returned almost nothing on-topic (deterministic A/B: baseline **23%** on-finding), so synthesis had only off-topic scraps; the grounding/evidence/scope nets check provenance, sufficiency and medical-ness but NOT *aboutness*. See test/results/DDX_FIX_FINDINGS.md + reproducible offline harness test/ab_ddx_retrieval.py (NCBI-only, no LLM/tokens).
- **ANALYSIS_TRUNCATION_FIX_ENABLED** (F1) — the rich analysis JSON overran a **hardcoded 512-token** cap on the `_analyze_and_expand_query` Haiku call for complex/differential queries → truncated mid-JSON → `json.loads` failed → SILENT fallback to the crude classifier (losing entities & pubmed_terms). Raises the budget to `ANALYSIS_MAX_TOKENS` (1280) AND salvages a truncated object (`_recover_truncated_json` / `_parse_analysis_json` keep the head fields — query_type/entities/rewrite — emitted first) so the pubmed_terms tail being cut no longer nukes the whole analysis. Verified deterministic 3/3 repro turned green. Dev true / prod false.
- **DIFFERENTIAL_DX_ENABLED** (F3) — TWO halves. (a) **Retrieval:** `differential_dx.differential_terms()` detects "what could this finding be?" queries (`is_differential_query`) and adds etiology/differential-sense PubMed terms anchored on the FINDING (mirrors query_sense for causation) — RECALL lever, A/B on-finding 23%→45%. (b) **Answer shape:** `prompt_engine._maybe_differential_guidance()` appends DIFFERENTIAL-DIAGNOSIS MODE guidance to the BLUF `dynamic_system` (both `build_bluf_only_messages` and `build_complex_bluf_messages`) so the answer is a RANKED differential (candidate diagnoses / discriminating features / red-flags / recommended workup) instead of the drug-centric complex framing. Prompt-level A/B (test/ab_f3_prompt.py): injected on 7/7 ddx queries, 0 leaks into 7 normal queries, absent with flag OFF. Answer *quality* (does the model follow it) still needs an LLM-judge run (tokens). Dev true / prod false.
- **TOPICALITY_GATE_ENABLED** (F2) — the missing *aboutness* net. `ranking.apply_topicality_gate()` keeps ONLY subject-mentioning articles (prefix-stem match so metastasis⇄metastases) and, unlike the relevance floor, NEVER re-admits off-topic ones to hit a count — empty result defers to the honest no_evidence card. Subject anchor = drug else first-entity, but ALL entities for comparative & differential queries (the finding). **Step-aside safety:** drops the off-subject remainder only when ≥1 article is on-subject; a total blank returns the set untouched (defers to evidence-floor/grounding-gate) so it never manufactures a spurious no_evidence card. PRECISION lever. A/B: F3+F2 = 91% on-finding, **0% off-topic noise**, 11 real splenic-metastasis articles. Broad sweep across 9 query shapes (test/ab_sweep.py): 100% on-subject after gate, **0 queries blanked**, retention 62–100%. Dev true / prod false.
- **F4 (no flag)** — `/api/v1/providers` added to `firebase_auth.EXEMPT_PATHS`. It is a public, secret-free catalog the frontend model-picker reads on page load (like `/config/llm`) but was auth-walled → 401 → picker never populated → an explicit engine pick (e.g. Haiku) was never sent as `model_explicit` → generation silently defaulted to Cerebras. This is the "Haiku not working" root cause; key/model/routing were all fine.

### RAGnosis-driven retrieval levers (2026-07-21, dev-first, prod OFF) — see test/results/RAGNOSIS_FINDINGS.md
Benchmarked med-ai on RAGnosis MRCP MCQs (Claude Code judges, no BYOK). Result at 120 questions: **120/120 faithful, 0 hallucinations**, 27% correct / 37% w/ partial. Safety is perfect at scale; the ceiling is retrieval + corpus-fit. Levers added:
- **INTENT_FRAMING_ENABLED** — `intent_framing.intent_terms(intent, entities)` threads the analyzer's existing `intent` into `pubmed_expansion_terms` (same plumbing as query_sense/differential_dx). HIGH-PRECISION intents only (diagnosis, drug_dosing, side_effect, contraindication) — broad intents diluted retrieval. Cut intent-mismatch 18→7. Wired in `process_query` next to the query_sense/differential hooks.
- **REFERENCE_FIRST_ENABLED** — makes StatPearls/NCBI-Books the answer source for fact/diagnosis questions. (a) `data_fetcher._americanize_term()` British→US spelling variant (haemochromatosis→hemochromatosis) for the book title search; (b) key-aware exponential backoff + jitter in `_ncbi_eutils_get` (3 retries with api_key, 1 without — see PLAYBOOK "NCBI E-utilities": 3/s no-key, 10/s per-key, 429 on breach, Retry-After unreliable); (c) **whole-chapter import** — `fetch_disease_data` passes `max_chapters=1, char_cap=60000` so the answer deep in a chapter grounds (t(15;17) is in the APL Etiology section, past the old 16k cut); (d) **concept-level title gate** in `_fetch_book_monographs` — a chapter must contain the entity's most DISTINCTIVE token (longest content word) OR ≥60% of its meaningful tokens; `_GENERIC_TITLE_TOKENS` (colour/laterality/acuity/filler) never qualify alone. Killed "black eschar→Black Piedra", "molluscum→HIV-Prevention", "myelopathy→external-ear"; grounded-but-wrong 3→0; StatPearls coverage 14%→42%.
- **CONTRADICTION_SURFACING_ENABLED** — `prompt_engine._maybe_contradiction_rule()` appends a rule to the BLUF + section prompts: when retrieved sources disagree, state the disagreement and cite both sides rather than silently picking one.
- **CANDIDATE_CHAPTERS_ENABLED** (COVERAGE lever, 2026-07-22) — the biggest correctness gain in the effort. Solves the symptom-vignette residual: for diagnostic queries the analyzer emits `candidate_diagnoses` (rule #13 in `_analyze_and_expand_query`, added to the prompt ONLY when the flag is on so prod-OFF stays byte-identical), and `rag_pipeline.process_query` threads them into `pubmed_expansion_terms["candidate_diagnoses"]` (same propagation channel as `comorbidity_list`). `data_fetcher.fetch_disease_data` then fetches a StatPearls chapter for the top 2 candidates (concurrently with the primary book task; `max_chapters=1, char_cap=40000` each), merges into `book_monographs` (dedup by nbk_id/title, cap 3 total). The concept-level title gate validates each candidate chapter against ITS candidate name, so only on-concept chapters survive. **Chapter ranking + budget:** merged chapters are ordered best-first by `data_fetcher._rank_book_monographs` (fraction of the chapter title that matches the query signal — disease name + candidates; primary keeps its lead on ties), and `prompt_engine._format_monographs` fills them to `settings.book_monograph_char_budget` (90k chars) so the most-relevant chapter is always whole and total grounding is bounded (≤3 chapters could otherwise reach ~140k chars / 35k tokens; only the least-relevant overflow chapter is head-truncated). **Additive + safe:** a wrong candidate's chapter lacks the fact → the pipeline abstains (never invents). RAGnosis 120-q (test/results/RAGNOSIS_FINDINGS.md): chapter coverage 38%→51%, overall correct 28%→48%, and on the 47 questions the lever fired the correct rate went **19%→72% (+25)** while the 73-question control cohort drifted −2 — so the gain is the lever, not judge variance. Safety unchanged (120/120 faithful, 0 hallucinations). Gated on a diagnostic query (`is_differential_query` OR intent=='diagnosis'). Requires `reference_first_enabled`. **`prompt_version` bumped 4→5** so enabling the flag busts stale cached analyses that lack `candidate_diagnoses`. Dev true / prod false. Caveat: candidate QUALITY depends on the production analyzer LLM (only fully measurable with BYOK); the offline test used a Claude analyzer stand-in.
...
### New Env Vars
- `OPENROUTER_CALLBACK_BASE` — base URL for OAuth callback (e.g. `https://med.kayomarz.com`)
