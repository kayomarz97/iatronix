...
| `ADAPTIVE_CROSS_STRATEGY_FALLBACK_ENABLED` | `true` | `false` | Confidence-gated cross-strategy fallback: when a typed fetch returns fewer than `CROSS_STRATEGY_MIN_UNIQUE_HITS` (default 3) DISTINCT articles, fire ONE complementary strategy (a bounded slice of "fetch-all"). Leaves well-served queries untouched. See `test/results/FINDINGS.md` |
