...
**Query-analysis cache (R6, `CLASSIFICATION_CACHE_ENABLED`).** Separate from the response cache above: `cache.analysis_cache_get/set` cache the full `_analyze_and_expand_query` result (query_type + entities + rewritten_query + pubmed_terms) keyed by `analysis:v{prompt_version}:{sha256(normalized_query)}` (TTL `CLASSIFICATION_CACHE_TTL_SECONDS`, 24h). `_analyze_and_expand_query_cached()` wraps the analyzer in the `process_query` gather, so identical queries skip the Haiku analysis call. Busted by `prompt_version`. **The analyzer also emits `candidate_diagnoses` when `CANDIDATE_CHAPTERS_ENABLED` (coverage lever — see AGENT_ARCHITECTURE.md); `prompt_version` was bumped 4→5 so enabling that flag invalidates cached analyses that predate the field.** Whenever the analyzer prompt or its output schema changes, bump `prompt_version` — a stale analysis cache silently starves a new retrieval lever. GOTCHA: `PROMPT_VERSION` is set as an **env var** in `.env.dev` / prod `.env`, which OVERRIDES the `config.py` default — bump the env var too (dev `.env.dev` is bumped to 5; prod `.env` must be bumped to 5 when the coverage lever is promoted), not just the code default.
...
| `RELEVANCE_FLOOR_ENABLED` | `false`* | `false` | Drop articles whose entity-relevance is 0 (drug/subject absent), keeping ≥ `RELEVANCE_FLOOR_MIN_KEEP` (3). Stops off-topic keyword matches. *Pipeline wiring pending — see `test/results/RELEVANCE_FINDINGS.md` |
| `RELEVANCE_SYNONYMS_ENABLED` | `false`* | `false` | Match entity synonyms in the floor (paracetamol⇄acetaminophen) so legit articles aren't wrongly dropped |
| `QUERY_SENSE_FRAMING_ENABLED` | `false`* | `false` | For causation queries ("does X cause Y") add adverse-sense PubMed terms so retrieval targets the cause sense, not the indication sense. Winning combo (factorial): floor+sense+synonyms |
